...
LOCATION_HEADERS = [
    ...
    'internal_dialing_enabled',
    'unknown_extension_route_id',
    'unknown_extension_route_name',
    'unknown_extension_route_type',
]
            ...
            json.dumps(
                {
                    ...
                    'outgoing_profiles': {
                        'profile_2': {
                            ...
                            'calling_permissions': {
                                'internal_call': {'action': 'ALLOW', 'transferEnabled': True},
                                'local': {'action': 'ALLOW', 'transferEnabled': True},
                                'national': {'action': 'ALLOW', 'transferEnabled': True},
                                'toll_free': {'action': 'ALLOW', 'transferEnabled': True},
                                'mobile': {'action': 'ALLOW', 'transferEnabled': True},
                                'international': {'action': 'BLOCK', 'transferEnabled': False},
                            },
                        }
                    },
                },
...
def load_locations(path: Path) -> list[LocationInput]:
    ...
    for row_number, row in enumerate(rows, start=2):
        ...
        payload['internal_dialing_enabled'] = _normalize_bool(payload.get('internal_dialing_enabled'))
        data.append(
            LocationInput(
