...
### Estado actual de v2.1 (qué faltaba y qué se completó)

Se completó el paso que faltaba de la versión anterior: pasar de maqueta/no-op a ejecución real por etapa para las acciones dentro de scope de v2.1.

Implementado ahora en `Space_OdT/v21/engine.py`:

- **Sedes (Location)**
  - Crear ubicación y activar Calling.
  - Resolver Route Group por `route_group_id` o `route_group_name`.
  - Configurar PSTN con Route Group.
  - Alta de numeraciones en sede en estado inactivo (`location_numbers`).
  - Asignar número cabecera (`main_number`).
  - Configurar dialing interno (`internal_dialing_enabled`, `unknown_extension_route_*`).
  - Configurar permisos salientes base de sede por perfil.

- **Usuarios**
  - Alta de número secundario legacy.
  - Configuración de desvío legacy (prefijo 53 o destino explícito).
  - Override de permisos salientes por perfil.
...
- **UI HTML5**
  - Botón por acción con preview/apply.
