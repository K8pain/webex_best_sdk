...
## Paso -1: preparar dependencias reales en el laboratorio (usuarios + IDs)

```bash
export WEBEX_ACCESS_TOKEN="tu_token_real"
python actions/bootstrap_lab_dependencies.py   --location-id "<location_id_real_del_lab>"   --users-count 3   --output-json actions/lab_bootstrap_output.json   --output-commands actions/script_test_commands_lab.md
```

> Usa después `actions/script_test_commands_lab.md`, que ya queda generado con IDs reales (usuarios creados/encontrados) y payloads por script.
